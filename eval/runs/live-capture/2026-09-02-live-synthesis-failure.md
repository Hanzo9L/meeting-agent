...
### 2026-09-03 synthesis ceiling and extractive decision

The live synthesis shape was corrected to allow multiple bullets per facet,
raise the schema and runtime bullet caps from 4 to 12, and request one short
ordered step per bullet. The frozen benchmark harness cap was also aligned to
12. In the prompt-shape run, the model emitted exactly 12 bullets on all six
requests, proving that the model follows the one-step-per-bullet instruction.

The final aligned medium-effort benchmark completed only 3 of 6 requests. All
three completed responses were schema-valid and binding-valid; the other three
hit the request timeout. Summary including timed-out runs:

    median 14,870ms   p95 15,037ms
    schema-valid 3/6  binding-valid 3/6
    errors: Request timed out 3/6

This is a synthesis latency ceiling, not a retrieval or validation failure. The
benchmark hard-codes a 15,000ms timeout in both the OpenAI client and synthesis
port, uses `maxRetries: 0`, and has no timeout CLI flag. Longer 12-bullet output
now regularly runs into that ceiling. Do not cite `schemaValid: 0` from the
earlier run: that was a stale harness-side `bullets.length > 4` check and has
been fixed.

Extractive measurements remain:

    baseline extractive          0.747ms p50   0 API calls
    procedural-probe extractive  1.441ms p50   0 API calls
    default extractive           1.271ms p50   0 API calls

At the current synthesis ceiling, the 0.747ms extractive baseline is roughly
20,000x faster. It has no step cap, cannot hallucinate beyond source spans, and
fails closed with explicit reasons. The extractive path is still not wired to
the live app.

Decision for the next session: preserve the corrected 12-bullet synthesis
contract, but do not resume synthesis latency tuning or vector-store migration
as the primary direction. Continue improving and wiring the deterministic
extractive path unless the product deliberately accepts a longer synthesis
timeout. Retrieval is already measured in milliseconds; the dominant delay and
new reliability ceiling are the single OpenAI synthesis call.
