...
import type {
  ...
  SharePointPowerShellCorpusJobRequest,
  SharePointPowerShellCorpusRunResult,
  SharePointPowerShellCorpusStats
} from "./types";

const SOURCE_ID = "ms-sharepoint-powershell";
const TRACK_ID = "ga";
const DEFAULT_ARTIFACTS_DIR = "eval/runs/indexing";
const REPO_PATH_PREFIX = "sharepoint/sharepoint-ps/Microsoft.Online.SharePoint.PowerShell/";
const LEARN_MODULE_SEGMENT = "microsoft.online.sharepoint.powershell";
...
/**
 * Deterministic Learn PowerShell-module canonical URL for a genuine SPO*
 * cmdlet doc (mirrors `resolveCanonicalCitationUrl`'s registry-driven
 * resolver so ingestion-time `canonicalUrl` and answer-time citation
 * resolution agree). Falls back to the GitHub blob URL when the path is not
 * a single cmdlet-reference file directly under the module directory.
 */
export function mapSharePointPowerShellPathToLearnUrl(sourcePath: string): string | null {
  const normalized = sourcePath.replace(/\\/g, "/").replace(/^\/+/, "");
  if (!normalized.toLowerCase().startsWith(REPO_PATH_PREFIX.toLowerCase())) return null;
  if (!normalized.toLowerCase().endsWith(".md")) return null;
  const rest = normalized.slice(REPO_PATH_PREFIX.length).replace(/\.md$/i, "");
  if (!rest || rest.includes("/") || rest.includes("..")) return null;
  if (!/^(?:Get|Set|Grant|Revoke|Remove|New|Test|Enable|Disable|Add|Repair|Start|Stop|Register|Unregister|Approve|Deny|Request|Restore|Move|Rename|Connect|Disconnect|Import|Export|Invoke|Recover|Submit|Update|Upgrade)-SPO[A-Za-z0-9]+$/.test(
    rest
  )) {
    return null;
  }
  return `https://learn.microsoft.com/powershell/module/${LEARN_MODULE_SEGMENT}/${rest.toLowerCase()}`;
}

export class SharePointPowerShellCorpusJob {
  ...
  async run(
    request: SharePointPowerShellCorpusJobRequest
  ): Promise<SharePointPowerShellCorpusRunResult> {
    const startedAt = new Date();
    const started = performance.now();
    const runId = `k2-sharepoint-powershell-${startedAt.toISOString().replace(/[:.]/g, "-")}`;
    ...
    if (!source || source.acquisition.transport !== "github") {
      throw new Error("Invalid source registry state for ms-sharepoint-powershell github source.");
    }
    ...
    if (request.mode === "execute" && !cancelled) {
      ...
      for (const doc of acquiredDocs) {
        ...
        process.stdout.write(
          `[K2-SPO-PS] completed=${done}/${acquiredDocs.length} failed=${failures.length} current=${currentPath} chunks=${indexed.chunks.newCount} embGen=${indexed.embeddings.generatedCount} embReuse=${indexed.embeddings.reusedCount}\n`
        );
    ...
    const smoke =
      request.mode === "execute"
        ...
        : {
            spoSiteSharing: null,
            restrictedContentDiscoveryCmdlet: null,
            teamsNegativeControl: null
          };
    ...
    const result: SharePointPowerShellCorpusRunResult = {
      runId,
      mode: request.mode,
  ...
  private async planIndexing(params: {
    ...
    return {
      ...
      estimatedDocumentsByType: { cmdlet: params.docs.length, conceptual: 0, other: 0 },
...
function canonicalUrlForDescriptor(descriptor: SourceFileDescriptor): string {
  return mapSharePointPowerShellPathToLearnUrl(descriptor.path) ?? descriptor.githubUrl;
}
...
async function writeArtifacts(result: SharePointPowerShellCorpusRunResult): Promise<void> {
  const artifact = {
    ...result,
...
function summarizeAcquisition(
  result: SharePointPowerShellCorpusRunResult["acquisitionResult"]
...
async function runRetrievalSmokes(params: {
  ...
  return {
    spoSiteSharing: await runSmokeCase({
      question: "What does the Set-SPOSite cmdlet do?",
      ...params
    }),
    restrictedContentDiscoveryCmdlet: await runSmokeCase({
      question: "How do I check Restricted Content Discovery status for a site using PowerShell?",
      ...params
    }),
    teamsNegativeControl: await runSmokeCase({
...
function renderMarkdownSummary(result: SharePointPowerShellCorpusRunResult): string {
  const lines: string[] = [];
  lines.push(`# K2 SharePoint PowerShell Run ${result.runId}`);
  lines.push("");
