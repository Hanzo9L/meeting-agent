...
import { LocalMarkdownCorpusJob } from "./localMarkdownCorpusJob";

const SOURCE_ID = "teams_ps_recipes";
const CORPUS_ROOT = "data/corpus/teams_ps_recipes";
...
async function main(): Promise<void> {
  ...
  const job = new LocalMarkdownCorpusJob({
    sourceId: SOURCE_ID,
...
main().catch((error) => {
  process.stderr.write(
    `${JSON.stringify({ error: error instanceof Error ? error.message : "teams_recipes_corpus_failed" }, null, 2)}\n`
  );
  process.exitCode = 1;
});
