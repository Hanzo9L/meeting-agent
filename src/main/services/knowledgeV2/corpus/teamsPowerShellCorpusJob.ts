...
import type {
  ...
  CorpusPlanEstimates,
  CorpusRunArtifacts,
  PowerShellCorpusStats,
  RetrievalSmokeCaseResult,
  TeamsPowerShellCorpusJobRequest,
  TeamsPowerShellCorpusRunResult
} from "./types";

const SOURCE_ID = "ms-teams-powershell";
const TRACK_ID = "ga";
...
export class TeamsPowerShellCorpusJob {
  ...
  async run(request: TeamsPowerShellCorpusJobRequest): Promise<TeamsPowerShellCorpusRunResult> {
    const startedAt = new Date();
    const started = performance.now();
    const runId = `cg01d-${startedAt.toISOString().replace(/[:.]/g, "-")}`;
    ...
    if (!source || source.acquisition.transport !== "github") {
      throw new Error("Invalid source registry state for ms-teams-powershell github source.");
    }
    ...
    if (request.mode === "execute" && !cancelled) {
      ...
      for (const doc of acquiredDocs) {
        ...
        process.stdout.write(
          `[CG-01D] completed=${done}/${acquiredDocs.length} failed=${failures.length} current=${doc.revision.transport === "github" ? doc.revision.path : doc.canonicalUrl} chunks=${indexed.chunks.newCount} embGen=${indexed.embeddings.generatedCount} embReuse=${indexed.embeddings.reusedCount}\n`
        );
        if (indexed.readiness === "failed") {
          const sourcePath = doc.revision.transport === "github" ? doc.revision.path : doc.canonicalUrl;
          failures.push({
            sourcePath,
          ...
          !store.tombstoneDocument(
            {
              ...
              trackId: TRACK_ID,
              transport: "github",
              canonicalUrl: deleted.githubUrl,
    ...
    const smoke =
      request.mode === "execute"
        ...
        : {
            setCsOnlineVoiceRoutingPolicy: null,
            conceptualVoiceRouting: null,
            unrelatedCmdlet: null
          };
    ...
    const result: TeamsPowerShellCorpusRunResult = {
      runId,
      mode: request.mode,
      ...
      if (descriptor.contentStatus !== "available" || typeof descriptor.content !== "string") {
        return {
          failure: {
            ...
            canonicalUrl: descriptor.githubUrl,
            stage: "acquisition",
  ...
  private async planIndexing(params: {
    ...
    for (const doc of params.docs) {
      ...
      const kind = classifyPowerShellDocumentType(sourcePath);
      byType[kind] += 1;
    }
    return {
...
function classifyPowerShellDocumentType(
  sourcePath: string
): "cmdlet" | "conceptual" | "other" {
  if (sourcePath.startsWith("teams/teams-ps/MicrosoftTeams/")) return "cmdlet";
  if (sourcePath.startsWith("teams/docs-conceptual/")) return "conceptual";
  return "other";
}
...
async function writeArtifacts(result: TeamsPowerShellCorpusRunResult): Promise<void> {
...
function computeCorpusStats(params: {
  ...
  const conceptualDocs = docs.filter((d) => d.sourcePath.startsWith("teams/docs-conceptual/")).length;
  const cmdletDocs = docs.filter((d) => d.sourcePath.startsWith("teams/teams-ps/MicrosoftTeams/")).length;
...
async function runRetrievalSmokes(params: {
  ...
  const setCsQuestion = "What does Set-CsOnlineVoiceRoutingPolicy do?";
  const conceptualQuestion = "How do voice routing policy and PSTN usages work in Teams PowerShell?";
  const unrelatedQuestion = "What does Get-CsOnlineUser do?";
  return {
    setCsOnlineVoiceRoutingPolicy: await runSmokeCase({
      question: setCsQuestion,
      ...params
    }),
    conceptualVoiceRouting: await runSmokeCase({
      question: conceptualQuestion,
      ...params
    }),
    unrelatedCmdlet: await runSmokeCase({
...
function renderMarkdownSummary(result: TeamsPowerShellCorpusRunResult): string {
  const lines: string[] = [];
  lines.push(`# CG-01D Run ${result.runId}`);
  lines.push("");
